...
import { TripCategories, getTripCategoryById } from '@/constants/theme';
...
export default function AddTripExpenseSheet({ sheetRef, tripId, tripName = 'Trip' }: Props) {
  ...
  const cat     = getTripCategoryById(category);
  ...
  const handleSave = async () => {
    ...
    const txnNote      = `${tripName} - ${name.trim()}`;
      ...
      <BottomSheet
        ...
        <BottomSheetScrollView
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow} keyboardShouldPersistTaps="handled">
              {accounts.map((acc) => {
                ...
                return (
                  <TouchableOpacity
                    key={acc.id}
                    style={[styles.chip, selected && styles.chipBank]}
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsRow} keyboardShouldPersistTaps="handled">
              {cards.map((card) => {
                ...
                return (
                  <TouchableOpacity
                    key={card.id}
                    style={[styles.chip, selected && styles.chipCard]}
          ...
          <Pressable style={styles.dateRow} onPress={() => { Keyboard.dismiss(); setShowDatePicker(true); }}>
            <MaterialCommunityIcons name="calendar-outline" size={16} color={C.trip} />
            <Text style={[styles.dateText, { color: C.trip }]}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
            disabled={!canSave}>
            <MaterialCommunityIcons name="check" size={20} color={C.onTrip} />
...
const styles = StyleSheet.create({
  ...
  currencySymbol: { color: C.trip, fontSize: 32, fontWeight: '700' },
  amountInput: {
    color: C.text,
    fontSize: 48,
    fontWeight: '800',
  ...
  chipsRow: { gap: 8, paddingBottom: 14, paddingRight: 4 },
  chip: {
  ...
  datePickerDone: {
    ...
    backgroundColor: C.trip, borderRadius: 12, marginTop: 8,
  },
  datePickerDoneText: { color: C.onTrip, fontWeight: '700', fontSize: 15 },
