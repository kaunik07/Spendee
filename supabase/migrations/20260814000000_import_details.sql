-- Statement import — carry per-subcategory detail (airline, cab provider,
-- restaurant, store...) into the expenses this import creates.
--
-- expenses.details already exists (20260721000000_initial_schema.sql) and is
-- already read/written by the manual-entry path (store/useExpenses.ts). This
-- migration is the statement-import RPC catching up to a column that was
-- already there — no new column, just CREATE OR REPLACE on a JSONB parameter
-- that was always structurally free to carry one more key.
